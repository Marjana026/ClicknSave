...
export async function POST(request){
    try {
        ...
        if (!userExists) {
            // Create user in the database using the form data
            await prisma.user.create({
                data: {
                    id: userId,
                    name: name, // Use the store owner's name
                    email: email, // Use the email from the form
                    image: "https://api.dicebear.com/7.x/initials/svg?seed=" + name, // Generate avatar from name
                    cart: {}
                }
            });
        }
